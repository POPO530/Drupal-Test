    ...
    attach: function (context, settings) {
      ...
      const resetGame = () => {
        ...
        for (let prize in prizeCounts) {
          prizeCounts[prize] = prize < 'F' ? 1 : 15; // 賞品数をリセット
      ...
      const drawTickets = () => {
        ...
        let drawCount = Math.min(remainingTickets, selectedCount);
        drawResult = [];
        revealed = []; // クリックされたくじを追跡する配列をリセット
      ...
      const revealTicket = (index) => {
        if (!revealed.includes(index)) {
          revealed.push(index);
          updatePrizeCounts(drawResult[index]); // 賞品の残り数を更新
          updateResultsDisplay();
          updatePrizeDisplay(); // 賞品の残り数表示を更新
        }
      };
          ...
          drawResult.forEach((result, index) => {
            ...
            resultElement.on('click', function() {
              if (!revealed.includes(index)) {
                ...
                $(this).off('click'); // このくじを再度クリックできないようにする
              }
            });
