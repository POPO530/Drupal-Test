...
(function ($, Drupal) {
  ...
  Drupal.behaviors.kujiGame = {
    ...
    const drawTickets = () => {
      ...
      updateAmountOfMoney(selectedCount);
    };
    ...
    const shuffleArray = (array) => {
      ...
      // 使用金額を計算する関数
      const updateAmountOfMoney = (selectedCount) => {
        let totalAmountDiv = $('#totalAmount', context);

        if (!totalAmountDiv.length) {
          $('body').append('<div id="totalAmount"></div>');
          totalAmountDiv = $('#totalAmount', context);
        }

        totalAmountDiv.empty().append('<h4>使用金額:</h4>');

        const totalAmount = amountOfMoney * selectedCount;

        prizeDisplayDiv.append(`${totalAmount}`);
      };
