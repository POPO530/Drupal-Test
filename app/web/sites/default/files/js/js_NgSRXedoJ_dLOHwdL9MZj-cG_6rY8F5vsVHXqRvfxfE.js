...
(function ($, Drupal) {
    // 'testBehavior'という名前のDrupalビヘイビアを定義します。
    Drupal.behaviors.testBehavior = {
      // 'attach'関数は、ページが読み込まれた時やAjax呼び出し後にDrupalによって自動的に呼び出されます。
      attach: function (context, settings) {
        // '.my-ajax-button'クラスを持つ要素に対してイベントリスナーを設定します。
        // 'context'と'.once'メソッドを使用して、このビヘイビアが一度だけ適用されることを保証します。
        $('.my-ajax-button', context).once('testBehavior').each(function () {
          $(this).click(function () {
            // ボタンがクリックされた時に実行される関数。
            $.ajax({
              url: '/test/ajax', // サーバーにリクエストを送るURL。
              method: 'GET', // HTTPメソッド。
              success: function (response) {
                // リクエストが成功した時に実行される関数。
                // サーバーからのレスポンス（ここではJSON形式のメッセージ）をアラートで表示します。
  ...
  })(jQuery, Drupal); // 自己実行関数の終わり。jQueryとDrupalオブジェクトを引数として渡します。;
/**
* DO NOT EDIT THIS FILE.
* See the following change record for more information,
* https://www.drupal.org/node/2815083
