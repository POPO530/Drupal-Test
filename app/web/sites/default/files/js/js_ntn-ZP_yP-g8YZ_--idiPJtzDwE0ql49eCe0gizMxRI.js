    ...
    Drupal.behaviors.kujiGame = {
      attach: function (context, settings) {
        ...
        const resetGame = () => {
          ...
          box = shuffle(initialBox);
          selectedCount = 0;
          drawResult = [];
          revealed = [];
          remainingTickets = initialBox.length;
          prizeCounts = { ...frequencies };
          $('#remainingTickets', context).text(remainingTickets);
          $('#results', context).html('');
        };
        ...
        const drawTickets = () => {
          selectedCount = parseInt($('#selectedCount', context).val(), 10) || 0;
          const updatedBox = [...box];
          const results = [];
          for (let i = 0; i < selectedCount; i++) {
            if (updatedBox.length === 0) break; // Check if the box is empty
            const ticketIndex = Math.floor(Math.random() * updatedBox.length);
            const ticket = updatedBox.splice(ticketIndex, 1)[0];
            results.push(ticket);
          }
          remainingTickets -= results.length;
          box = updatedBox;
          revealed = new Array(results.length).fill(false);
          tempResults = results;
          drawResult = new Array(results.length).fill('未開封');
        ...
        const revealTicket = (index) => {
          revealed[index] = true;
          drawResult[index] = tempResults[index];
          updateResultsDisplay();
        };
        ...
        const updateResultsDisplay = () => {
          ...
          drawResult.forEach((result, index) => {
            ...
            resultElement.on('click', () => revealTicket(index));
